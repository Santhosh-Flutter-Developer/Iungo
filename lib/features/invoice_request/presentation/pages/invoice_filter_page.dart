...
/// "Filter" screen for the Invoice Dashboard — two tabs, mirroring
/// `GrnFilterPage` exactly: "Filter" (Contract + Created-date range +
/// Apply) and "Find Ticket" (lookup by Invoice Number).
///
/// Driven by [InvoiceFilterControllerLike] rather than the concrete
...
class InvoiceFilterPage extends StatefulWidget {
  ...
  final InvoiceFilterControllerLike controller;

  static Future<void> show(
    BuildContext context, {
    required InvoiceFilterControllerLike controller,
  }) {
    return Get.to(() => InvoiceFilterPage(controller: controller)) ??
        Future.value();
  }
...
class _InvoiceFilterPageState extends State<InvoiceFilterPage>
  ...
  late InvoiceRequestFilter _draft = widget.controller.filter.value;
  ...
  void _clear() {
    setState(() {
      _draft = const InvoiceRequestFilter();
