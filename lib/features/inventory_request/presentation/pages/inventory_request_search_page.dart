...
/// Search screen for "Inventory Request → Awaiting Client Approval".
/// Deliberately identical UI to [PendingWorkOrderSearchPage] — white app
/// bar with an inline text field, a field-scope dropdown ("All Fields" /
/// "Ticket Id" / "Subject" / "Description"), and a close icon — matched
/// purely against the local seed list instead of the API.
class InventoryRequestSearchPage extends GetView<InventoryRequestSearchController> {
  const InventoryRequestSearchPage({super.key});
      ...
      body: SafeArea(
        ...
        child: Obx(() {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) => InventoryRequestCard(
              request: controller.results[index],
              onTap: () => Get.to(
                () => const InventoryRequestDetailPage(),
                binding: InventoryRequestDetailBinding(controller.results[index]),
              ),
            ),
          );
        }),
...
class _SearchAppBar extends GetView<InventoryRequestSearchController>
...
class _ScopeDropdownButton extends GetView<InventoryRequestSearchController> {
  const _ScopeDropdownButton();
