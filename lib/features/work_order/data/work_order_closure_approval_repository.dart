...
/// Single in-memory source of truth for "Awaiting Approval for Closure".
///
/// Mirrors [WorkOrderRepository] exactly, backed by the
/// `awaitingapprovalforclousre` view instead of `all`. Registered as a
/// permanent GetxService (not tied to a page's lifecycle) so the list
/// page and search screen share the same [workOrders] list.
///
...
/// [WorkOrderClosureApprovalRemoteDataSource.fetchWorkOrderDetail] — so
/// Detail View navigation reuses the existing, already-registered
/// `WorkOrderRepository` rather than duplicating that fetch here.
class WorkOrderClosureApprovalRepository extends GetxService {
  WorkOrderClosureApprovalRepository(
    this._remoteDataSource,
    this._pickListDataSource,
  );

  final WorkOrderClosureApprovalRemoteDataSource _remoteDataSource;
  ...
  /// Fetches one page of the base (unfiltered) "Awaiting Approval for
  /// Closure" list.
  ...
  /// Hits the server's `quickFilter`/`search` "Awaiting Approval for
  /// Closure" endpoint — used once a Status/Priority/Due-Date filter or
