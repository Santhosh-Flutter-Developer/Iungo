...
/// Talks to the Iungo/Facilio "Awaiting Approval for Closure" list APIs.
///
/// Mirrors [WorkOrderRemoteDataSourceImpl] (the "All Work Orders" data
/// source) exactly — same query-parameter shape, same response parsing,
/// same error handling — swapping only the `viewName`/`view` path
/// segment from `all` to `awaitingapprovalforclousre`. The Detail View
/// fetch is intentionally left pointed at the `view/all` endpoint,
/// unchanged from "All Work Orders": Facilio's detail record is
/// view-independent, so every Work Order list (including this one) opens
/// the exact same Detail View API/URL/model.
abstract class WorkOrderClosureApprovalRemoteDataSource {
  /// Fetches one page of "Awaiting Approval for Closure" (1-indexed
  /// [page]) — the base, unfiltered list. Confirmed shape:
  ///
  ///   GET .../v3/modules/workorder/view/awaitingapprovalforclousre?
  ///       fetchOnlyViewGroupColumn=true&moduleName=workorder&
  ///       viewName=awaitingapprovalforclousre&page=<page>&
  ...
  /// Hits the server's `quickFilter` "Awaiting Approval for Closure"
...
class WorkOrderClosureApprovalRemoteDataSourceImpl
    implements WorkOrderClosureApprovalRemoteDataSource {
  WorkOrderClosureApprovalRemoteDataSourceImpl(this._dio, this._session);
  ...
  static const _viewName = 'awaitingapprovalforclousre';
  static const _closureApprovalUrl = '$_baseUrl/view/$_viewName';
