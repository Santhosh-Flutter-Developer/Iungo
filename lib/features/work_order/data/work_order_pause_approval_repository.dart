...
/// Single in-memory source of truth for "Awaiting for Pause Approval".
///
/// Mirrors [WorkOrderClosureApprovalRepository] exactly, backed by the
/// `awaitingforpauseworkorders` view instead of
/// `awaitingapprovalforclousre`. Registered as a permanent GetxService
/// (not tied to a page's lifecycle) so the list page and search screen
/// share the same [workOrders] list.
///
/// Status/Priority pick-list options are shared with "All Work Orders"
/// (they aren't view-specific), so this repository is handed the same
/// [WorkOrderPickListRemoteDataSource] instance rather than a new one.
/// Detail View data (Tasks/Comments/Attachments/full record) is also
/// view-independent — see
/// [WorkOrderPauseApprovalRemoteDataSource.fetchWorkOrderDetail] — so
/// Detail View navigation reuses the existing, already-registered
/// `WorkOrderRepository` rather than duplicating that fetch here.
class WorkOrderPauseApprovalRepository extends GetxService {
  WorkOrderPauseApprovalRepository(
    this._remoteDataSource,
    this._pickListDataSource,
  );

  final WorkOrderPauseApprovalRemoteDataSource _remoteDataSource;
  ...
  /// Fetches one page of the base (unfiltered) "Awaiting for Pause
  /// Approval" list.
  ...
  /// Hits the server's `quickFilter`/`search` "Awaiting for Pause
  /// Approval" endpoint — used once a Status/Priority/Due-Date filter or
  /// a Find-Ticket lookup (by `serialNumber`) is active.
