...
/// Search screen for the GRN Dashboard — matches Number and/or Contract
...
class GrnSearchPage extends GetView<GrnSearchController> {
  const GrnSearchPage({super.key});
      ...
      body: SafeArea(
        ...
        child: Obx(() {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) => GrnRequestCard(
...
class _SearchAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  final GrnSearchController controller;
...
class _ScopeDropdownButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return PopupMenuButton<GrnSearchScope>(
      icon: const Icon(Icons.sort, color: Colors.grey),
      offset: const Offset(0, 40),
      color: AppColors.white,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(4)),
      onSelected: controller.onScopeChanged,
      itemBuilder: (context) => GrnSearchScope.values
          .map(
            (item) => PopupMenuItem<GrnSearchScope>(
              value: item,
              child: _ScopeMenuRow(
                scope: item,
